...
; registros:
    ...
    mov r12, rdi                      ; r12  = *ids
    mov dword[RBP-48], esi            ; dword[RBP-48] = cantidadDeIds
    ...
    xor r15, r15        ; r15d = índice = 0

.loop:
    cmp r15d, dword[RBP-48]              ; condición de corte
    je .devuelvoARREGLO

    mov rdi, USUARIO_SIZE                        ; rdi = USUARIO_SIZE
    call malloc   

    mov r13, rax

    mov qword[rbx + (r15*8)], r13                ; arreglo[i] = nuevo_usuario;

    mov r8d, dword[r12 + (r15*4)]                ; r8d = ids[i]
    mov dword[r13 + USUARIO_ID_OFFSET], r8d      ; nuevo_usuario->id = ids[i];

    mov edi, r8d        ; edi = ids[i]
    call r14

    mov byte[r13 + USUARIO_NIVEL_OFFSET], al      ; nuevo_usuario->nivel = deQueNivelEs(ids[i]);
...
; Versión que usa pila pero paara otro parametro (quizás más eficiente)
